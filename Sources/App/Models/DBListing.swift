...
extension BDXSubdivision {
    func toDbListing(with builderID: Int) -> DBListing {
        ...
        return DBListing(listing: name,
                         ...
                         bedLow: plans?.reduce(plans?.first?.bedrooms ?? 0, {min($0, $1.bedrooms)}) ?? 0,
                         bedHigh: plans?.reduce(plans?.first?.bedrooms ?? 0, {max($0, $1.bedrooms)}) ?? 0,
                         bathLow: Float(plans?.reduce(plans?.first?.bathrooms ?? 0, {min($0, $1.bathrooms)}) ?? 0),
                         bathHigh: Float(plans?.reduce(plans?.first?.bathrooms ?? 0, {max($0, $1.bathrooms)}) ?? 0),
